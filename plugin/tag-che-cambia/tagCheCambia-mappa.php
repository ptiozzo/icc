...
$argsMappaTagCheCambiaSlider = array(
...
$loopMappaTagCheCambiaSlider = new WP_Query( $argsMappaTagCheCambiaSlider );

if ($loopMappaTagCheCambiaSlider->have_posts()) {
  $loopMappaTagCheCambiaSlider->the_post();
  ...
  <div class='head'>
    <div class='title'>
      <h5>LA MAPPA DI <?php echo strtoupper(get_term_by('slug', $tagPage,'post_tag')->name); ?></h5>
    </div>
  </div>
  ...
  <div id="carouselMappa" class="carousel carousel-control-top slide" data-ride="carousel" data-interval="5000">
    <div class="slider-top bg-dark d-flex flex-row align-items-center justify-content-between mb-2">
      ...
      <ol class="carousel-indicators pr-2 text-white">
        ...
        <?php
        $i = 1;
        while ($loopMappaTagCheCambiaSlider->have_posts()) {
          $loopMappaTagCheCambiaSlider->the_post();
          ?>
