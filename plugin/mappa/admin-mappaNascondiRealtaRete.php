<h1>Mappa Nascondi Realtà Rete</h1>

<?php
/* ---- Rimuovi nuova voce ai Rete ---- */
if ($_POST['icc_RealtaReteNascosta_attivi_Rimuovi']){
  ...
  foreach ($RetiNascoste as $Rete){
    if($_POST['icc_RealtaReteNascosta_attivi_Rimuovi'] != "Rimuovi ".$Rete['ReteSlug']){
  ...
  update_option('icc_RealtaReteNascosta_attivi',$RetiNascosteNew,'no');
}
?>

<!-- Aggiungo nuova voce ai Rete -->
<?php $RetiNascoste = get_option("icc_RealtaReteNascosta_attivi") ? get_option("icc_RealtaReteNascosta_attivi") : array(); ?>
<?php
  if($_POST["icc_RetiNascoste_Add"]){

    $RetiNascoste = get_option("icc_RealtaReteNascosta_attivi") ? get_option("icc_RealtaReteNascosta_attivi") : array();
    $RetiNascoste[] = array('ReteSlug' => $_POST['icc_RealtaReteNascosta_attivi']);

    update_option('icc_RealtaReteNascosta_attivi',$RetiNascoste,'no');
  }
...
foreach ($RetiNascoste as $Rete) {
  ...
  echo '<input type="submit" name="icc_RealtaReteNascosta_attivi_Rimuovi" value="Rimuovi '.$Rete['ReteSlug'].'" class="button">';
  echo "<br>";
}
 ?>
 <br>
